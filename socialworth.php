...
function apiCall ($method, $url, $params = array())
{
    ...
    if ($api) {
        ...
        if (isset($params['headers'])) {
            curl_setopt($api, CURLOPT_HTTPHEADER, $params['headers']);
        }
        ...
        if ($method == 'GET') {
            ...
            if (isset($params['payload'])) {
                curl_setopt($api, CURLOPT_POSTFIELDS, $params['payload']);
            }
...
foreach ($endpoints as $endpoint) {
    ...
    if ($endpoint['name'] == 'mozscape' && (!SEOMOZ_ID || !SEOMOZ_KEY)) {
        continue;
    }

    if (isset($endpoint['headers'])) {
        $params['headers'] = $endpoint['headers'];
    }

    if (isset($endpoint['payload'])) {
        $params['payload'] = $endpoint['payload'];
    }
